...
public class OperationUtility {
    ...
    private static void updateUser(UserDao userDao) {
        ...
        user.setEmail("usernewemail@email.com");
        userDao.save(user);
    }

    private static void deleteUser(UserDao userDao) {
        User user = userDao.findById(3L).orElseThrow(
                () -> new EntityNotFoundException("User not found"));
        userDao.delete(user);
    }
    ...
    private static void createCourses(CourseDao courseDao, InstructorDao instructorDao) {
        ...
        Course course1 = new Course("Hibernate Course", "5 hours", "Introduction to hibernate", instructor);
